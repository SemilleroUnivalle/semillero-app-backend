...
hoja_vida_base = "hoja_vida.pdf"
cert_laboral_base = "certificado_laboral.pdf"
cert_academico_base = "certificado_academico.pdf"
foto_base = "foto.png"
...
grados_escolaridad = ["Normal", "Especialización", "Maestría", "Doctorado"]
...
def create_user_safe(username, email, first_name, last_name, password):
    ...
    try:
        ...
        if hasattr(UserModel.objects, "create_user"):
            ...
            user = UserModel(username=username, email=email, first_name=first_name, last_name=last_name, user_type="profesor")
...
def main():
    ...
    for i in range(N):
        ...
        grado_escolaridad = random.choice(grados_escolaridad)
        ...
        hoja_vida_path = os.path.join(base_dir, hoja_vida_base)
        cert_laboral_path = os.path.join(base_dir, cert_laboral_base)
        cert_academico_path = os.path.join(base_dir, cert_academico_base)
        ...
        try:
            with transaction.atomic():
                ...
                if modulo_instance is not None:
                    try:
                        profesor.modulo = modulo_instance
                    except Exception:
                ...
                if ensure_file(doc_id_path):
                    with open(doc_id_path, "rb") as f:
                        profesor.documento_identidad_pdf.save(f"{pk}_{doc_id_base}", File(f), save=False)

                if ensure_file(rut_path):
                    with open(rut_path, "rb") as f:
                        profesor.rut_pdf.save(f"{pk}_{rut_base}", File(f), save=False)

                if ensure_file(cert_bancario_path):
                    with open(cert_bancario_path, "rb") as f:
                        profesor.certificado_bancario_pdf.save(f"{pk}_{cert_bancario_base}", File(f), save=False)

                if ensure_file(hoja_vida_path):
                    with open(hoja_vida_path, "rb") as f:
                        profesor.hoja_vida_pdf.save(f"{pk}_{hoja_vida_base}", File(f), save=False)

                if ensure_file(cert_laboral_path):
                    with open(cert_laboral_path, "rb") as f:
                        profesor.certificado_laboral_pdf.save(f"{pk}_{cert_laboral_base}", File(f), save=False)

                if ensure_file(cert_academico_path):
                    with open(cert_academico_path, "rb") as f:
                        profesor.certificado_academico_pdf.save(f"{pk}_{cert_academico_base}", File(f), save=False)

                if ensure_file(foto_path):
                    with open(foto_path, "rb") as f:
                        profesor.foto.save(f"{pk}_{foto_base}", File(f), save=False)

                # Guardar instancia Profesor (y con ello las relaciones a archivos)
                profesor.save()
                created += 1
                print(f"{i+1}/{N}: Usuario '{username}' y Profesor '{nombre} {apellido}' creados (PK Profesor: {profesor.pk}).")
        except Exception as e:
            print(f"ERROR creando registro {i+1} (numero_documento={numero_documento}): {e}")
            # continuar con los siguientes registros

    print(f"Fin: {created}/{N} profesores creados con usuarios asociados.")
